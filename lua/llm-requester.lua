...
local function create_split()
    ...
    api.nvim_buf_set_keymap(left_buf, 'n', '<leader>r', '', {
        callback = M.send_request,
        desc = 'Send request to Ollama'
    })
    
    api.nvim_buf_set_keymap(left_buf, 'n', '<leader>q', '', {
        callback = function()
...
vim.keymap.set('v', '<leader>oc', M.open_code_window, { desc = 'Open LLMRequester window' })
